/** Ssx.java
 * Copyright 2010, 2011 Stephen D. Williams, OptimaLogic, and Client sdw@lig.net http://sdw.st
 * License: Apache 2.0
 *
...
/* Current limitations:
 * To be improved:
 *   Once namespaces are defined, they remain active for the rest of the parse.
 *   Parse but ignore comments currently.  Might want to expose.
 *   Lexical events are not fully implemented.
 *   Currently doesn't support modification.
 *
 * Possible improvements probably not desired:
 *   Only support basic set of entitities.
 *   Parse but ignore DTDs, PIs.
 */
...
// For the non-native SAX mode, the following imports are only used for interfaces.  Could be duplicated inline if needed.
...
public class Ssx {
    ...
    // enum S {
    // Top, LT, Xml, Start, StartRest, StartFinish, AttrName, AttrEqual, AttrVal, AttrNoQ, AttrSingle, AttrDouble, MaybeChars, Chars, Comment, End, EndEnd; }
    static final int Top=0, LT=1, Xml=2, Elem=3, ElemRest=4, ElemFinish=5, AttrName=6, AttrEqual=7, AttrVal=8, AttrNoQ=9, AttrSingle=10,
        AttrDouble=11, MaybeChars=12, Chars=13, Comment=14, End=15, EndEnd=16, DocType=17, CData=18;
    /**
    ...
    class SParser implements XMLReader {
        ...
        public ErrorHandler getErrorHandler() { return eh; }
        public boolean getFeature(String name) { return features.get(name); }
        ...
        // Amortized, lightweight growable character array.  Call stretch() every periodicCheck appends to avoid running out of space.
        class Cheapcs {
            char[] s; int i;
            public Cheapcs(int d){ s=new char[d]; i=0; }
        ...
        // Handle: <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" "http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">
        public void parse(String xs) throws SAXException {
            ...
            int periodic = 0;
            int docTypeNest = 0;
            try { // This has very condensed formatting for a reason.  Read each state like a sentence.
                for(;x < xlen;x++) {
                    if (x > periodic+periodicCheck) { ename.stretch(); aname.stretch(); aval.stretch(); chs.stretch(); periodic=x; }
                    c = xsc[x];
                    if ((c == ' ' || c == '\t' || c == '\r' || c == '\n')) space = true; else space = false;
                    if (space && eatSpace) continue;
                    switch (state) {
                    case Top:if (c == '<') { state = LT; startOfElement=x; } break;// XmlDecl or top element
                    case LT: if (notStarted) { ch.startDocument(); npush("_xml_"); notStarted = false; }
                        if (c == '!') {if (xsc[x+1]=='-' && xsc[x+2]=='-') { x=x+2; state=Comment; eatSpace=false; } else
                                if (cmatch(xsc,x+1,"DOCTYPE")) { x=x+8; state=DocType; docTypeNest=1; }
                        } else
                            if (c == '?') { inXmlDecl = true; state = Xml; } else if (c == '/') { state=End; } else {
                                if (c=='&') x=appendEntity(ename,xsc,x); else ename.s[ename.i++]=c; state = Elem; eatSpace=false; } break;
                    case Xml:if (c == 'x' && xsc[x+1]=='m' && xsc[x+2]=='l') { x+=2; state = ElemRest; } else
                            throw new SAXException("Only support: \"<?xml ...\"\n"+toXmlAll()); break;
                    case Elem: if (space || c=='>' || c=='/') { if (space) state=ElemRest; else {
                                if (c=='/') {state=ElemFinish;eatSpace=true;} else { state=MaybeChars; startElement(atts); npush(ename); ratts(); re(); } } } else
                            if (c=='&') x=appendEntity(ename,xsc,x); else ename.s[ename.i++]=c; break;
                    case ElemRest: // Everything after the element name
                        if (inXmlDecl && c == '?') { if (xsc[x+1] != '>') throw new SAXException("Malformed XML decl end, expected '>' after '?'\n"+toXmlAll());
                            x++; /*ch.startDocument(); notStarted=false;*/ state=Top; inXmlDecl=false; }
                        else if (c == '>') { state = MaybeChars; startElement(atts); npush(ename); ratts(); re();
                        } else if (c == '/') { state = ElemFinish;
                        } else { eatSpace = false; state = AttrName; if (c=='&') x=appendEntity(aname,xsc,x); else aname.s[aname.i++]=c; } break;
                    case ElemFinish: if (c == '>') { state = MaybeChars;
                            startElement(atts); /* npush(ename); npop(ename);*/ endElement(); ratts(); re(); } else
                            throw new SAXException("ElemFinish no '>'.\n"+toXmlAll()); break;
                    case AttrName:
                        if (space) { eatSpace = true; state = AttrEqual;
                        } else if (c == '=') { if (aname.i == 0) throw new SAXException("Attribute must have name.\n+toXmlAll()"); eatSpace = true; state = AttrVal;
                        } else if (c=='&') x=appendEntity(aname,xsc,x); else aname.s[aname.i++]=c; break;
                    case AttrEqual: if (c == '=') { state = AttrVal; } else throw new SAXException("Need '=' after attribute name.\n"+toXmlAll()); break;
                    case AttrVal: eatSpace = false; if (c == '\'') { state = AttrSingle; } else
                            if (c == '"') { state = AttrDouble; } else { if (c=='&') x=appendEntity(aval,xsc,x); else aval.s[aval.i++]=c; state = AttrNoQ; } break;
                    case AttrNoQ: if (space || c=='>') { state=ElemRest; if (c=='>') x--; eatSpace = true;
                            natts(); atts.put(new String(aname.s,0,aname.i),new String(aval.s,0,aval.i)); ra(); } else
                            if (c=='&') x=appendEntity(aval,xsc,x); else aval.s[aval.i++]=c; break;
                    case AttrSingle: if (c == '\'') { state = ElemRest; eatSpace = true; natts(); atts.put(new String(aname.s,0,aname.i),new String(aval.s,0,aval.i)); ra(); } else
                            if (c=='&') x=appendEntity(aval,xsc,x); else aval.s[aval.i++]=c; break;
                    case AttrDouble: if (c == '"') { state = ElemRest; eatSpace = true; natts(); atts.put(new String(aname.s,0,aname.i),new String(aval.s,0,aval.i)); ra(); } else
                            if (c=='&') x=appendEntity(aval,xsc,x); else aval.s[aval.i++]=c; break;
                    case MaybeChars: if (c=='<') { if (xsc[x+1]=='!' && cmatch(xsc,x+2,"[CDATA[")) {x=x+8; state=CData; eatSpace=false; } else {state=LT; startOfElement=x;} } else {
                            if (c=='&') x=appendEntity(chs,xsc,x); else chs.s[chs.i++]=c; state=Chars; eatSpace=false; } break;
                    case Chars: if (c=='<') { if (xsc[x+1]=='!' && cmatch(xsc,x+2,"[CDATA[")) {x=x+8; state=CData; eatSpace=false; } else 
                                { state=LT; startOfElement=x; eatSpace=true; if (chs.i>0) { ch.characters(chs.s, 0, chs.i); rc(); } }} else
                            if (c=='&') x=appendEntity(chs,xsc,x); else chs.s[chs.i++]=c; break;
                    case CData: if (c==']'&&xsc[x+1]==']'&&xsc[x+2]=='>') { x=x+2; state=Chars; } else if (c=='&') x=appendEntity(chs,xsc,x); else chs.s[chs.i++]=c; break;
                    case Comment: if (c=='-' && xsc[x+1]=='-' && xsc[x+2]=='>') { eatSpace=true; state=MaybeChars; } break;
                    case End: eatSpace = false; if (c=='>') {npop(ename);  endElement(); re(); state=MaybeChars; eatSpace=true; }
                        else if (space) { eatSpace=true; state=EndEnd; } else if (c=='&') x=appendEntity(ename,xsc,x); else ename.s[ename.i++]=c; break;
                    case EndEnd: if (c=='>') { npop(ename); endElement(); state=MaybeChars; re(); } else
                    ...
                    case DocType: if (c=='<') { docTypeNest++; /*capture*/} else if (c=='>') { docTypeNest--; if (docTypeNest<1) state=MaybeChars; else {/*capture*/}}
                        else {/*capture*/} break;
                    }
                }
                ch.endDocument(); npop("_xml_");
            } catch (Exception e) { e.printStackTrace(); throw new SAXException(e+"\nStopped at:\n"+toXmlAll()); }  // It's parsing all the way down.
        }
        /**
        ...
        int appendEntity(Cheapcs cs, char[] xsc, int x) throws SAXException {
            // Already at '&'
            if (x+5 >= xsc.length) throw new SAXException("Truncated input: not enough characters to parse entity.\n"+toXmlAll());
            char next = xsc[x+1];
            if (next=='l') if (new String(xsc, x+1, 3).equalsIgnoreCase("lt;")) {cs.s[cs.i++]='<'; return x+3;}
            if (next=='g') if (new String(xsc, x+1, 3).equalsIgnoreCase("gt;")) {cs.s[cs.i++]='>'; return x+3;}
            if (next=='a') if (xsc[x+2]=='m') { if (new String(xsc, x+1, 4).equalsIgnoreCase("amp;")) {cs.s[cs.i++]='&'; return x+4;}}
                else if (new String(xsc, x+1, 5).equalsIgnoreCase("apos;")) {cs.s[cs.i++]='\''; return x+5;}
            if (next=='q') if (new String(xsc, x+1, 5).equalsIgnoreCase("quot;")) {cs.s[cs.i++]='\"'; return x+5;}
            if (next=='#') {
                int dx = x+2, start;
                char[] c;
                if (xsc[x+2]=='x') {
                    dx = dx+1; start = dx;
                    while (xsc[dx]!=';' && dx-x < 8) dx++;
                    c = Character.toChars(Integer.parseInt(new String(xsc,start,dx-start),16));
                } else { start = dx;
                    while (Character.isDigit(xsc[dx]) && dx-x < 8) dx++;
            ...
            cs.s[cs.i++]='&';
            return x; // Fall through to just use '&' as a normal character since we couldn't parse the entity.
        }
        private void endElement() throws SAXException {
            String qn = new String(ename.s,0,ename.i);
            String ns="", ln="";
            int col = -1;
            for (int x=0; x<ename.i; x++) if (ename.s[x] == ':') { col=x; break; }
            if (col > -1) { ns = new String(ename.s,0,col); ln = new String(ename.s, col+1, ename.i-col-1); } else { ln=new String(ename.s,0,ename.i); }
        ...
        void npush(Cheapcs cs) { String s=new String(cs.s,0,cs.i); npush(s); }
        void npush(String s) { u2pm.push(s); p2um.push(s); }
        // Make ch.endPrefixMapping() calls here!
        String npop(Cheapcs cs) throws SAXException { return npop(new String(cs.s,0,cs.i)); }
        String npop(String s) throws SAXException { u2pm.pop(s); p2um.pop(s); return s; }
        void natts() { if (atts==null) atts=new SAttributes(); }
        void ratts() { if (atts!=null) atts.reset(); }
        void ra() { aname.i=0; aval.i=0; }
        void re() { ename.i=0; }
        void rc() { chs.i=0; }
        public void setContentHandler(ContentHandler handler) { ch = handler; }
        public void setDTDHandler(DTDHandler handler) { }
        public void setEntityResolver(EntityResolver resolver) { }
        public void setErrorHandler(ErrorHandler handler) { eh = handler; }
        public void setFeature(String name, boolean value) { features.put(name, value); }
        public void setProperty(String name, Object value) { properties.put(name, value); }
        public String toXmlAll() { return new String(xmlChars,0,x); }
        StackMap<String,String> u2pm = new StackMap<String,String>(); // Static relative to SAttributes. // namespace uri to prefix
        StackMap<String,String> p2um = new StackMap<String,String>(); // Static relative to SAttributes. // prefix to namespace uri
        class SAttributes implements org.xml.sax.Attributes {
            Map<String,Integer> qnIndex = new TreeMap<String,Integer>(/*10*/);
            class Attrib { String uri; String ln; String qn; String val; public Attrib(String u, String l, String q, String v) {qn=q;uri=u;ln=l;val=v;
            ...
            public void reset() { as.clear(); }
            public void put(String q, String val) throws SAXException {
                ns = ""; ln = q; qn = q; boolean wasXmlns = false;
                if ((col = qn.indexOf(':')) > -1) { ns = qn.substring(0,col); ln = qn.substring(col+1);
                    if (ns.equalsIgnoreCase("xmlns")) { p2um.put(ln, val); u2pm.put(val, ln); ch.startPrefixMapping(ln, val); wasXmlns=true; }
                } else if (qn.equalsIgnoreCase("xmlns")) { p2um.put("", val); u2pm.put(val, ""); ch.startPrefixMapping("", val); wasXmlns=true; }
                if (!wasXmlns) { ns=p2u(ns); if (ns.length()!=0) qn=ns+":"+ln; else qn=ln; as.add(new Attrib(ns, ln, qn, val)); qnIndex.put(qn, as.size()); }
            }
            // Go from namespace URI to namespace id.
            private String u2p(String ns) { String q = u2pm.get(ns); if (q == null) return ns; else return q; }
            // go from namespace id to namespace URI.
            private String p2u(String q) { String ns = p2um.get(q); if (ns == null) return q; else return ns; }
            public int getIndex(String qn) {
                ns = ""; ln = qn;
                if ((col = qn.indexOf(':')) > -1) { ns = qn.substring(0,col); ln = qn.substring(col+1); }
                return getIndex(ns, ln);
            }
            public int getIndex(String uri, String ln) { return qnIndex.get(p2u(uri)+":"+ln); }
            public int getLength() { return as.size(); }
            ...
            public String getValue(String uri, String ln) { return as.get(qnIndex.get(p2u(uri)+":"+ln)).val; }
            int lastIndex=-1; // Caching what we just looked up.
            Attrib lastAs=null;
            public String getLocalName(int index) { if (index!=lastIndex) { lastAs=as.get(index); lastIndex=index; } return lastAs.ln; }
            public String getQName(int index) { if (index!=lastIndex) { lastAs=as.get(index); lastIndex=index; } return lastAs.qn; }
            public String getURI(int index) { if (index!=lastIndex) { lastAs=as.get(index); lastIndex=index; } return lastAs.uri; }
            public String getValue(int index) { if (index!=lastIndex) { lastAs=as.get(index); lastIndex=index; } return lastAs.val; }
            // Unimplemented - need DTDs / schema for this.
            public String getType(int index) { return "string"; }
            public String getType(String uri, String ln) { return "string"; }
        } // End SAttributes
    ...
    public Xml parseUnused(String xmlString, String nsSet) throws IOException, ParseException, UnsupportedEncodingException {
    ...
    int elements=0;
    public static void setParseType(boolean sparsep, boolean defaultParserp, boolean timeAllp) { sparse=sparsep; defaultParser=defaultParserp; timeAll=timeAllp; }
        ...
        try {
            if (defaultParser) {
                ...
                if (timed||timeAll) {
                    ...
                    if (debug) verbose("Parser:Default Parsed "+what+" "+Integer.toString(len)+" characters, "+
            ...
            if (sparse) {
                ...
                if (timed||timeAll) {
                    now = System.currentTimeMillis();
                    if (debug) verbose("Parser:Sparse  Parsed "+what+" "+Integer.toString(len)+" characters, "+
                                       elements+" elements in "+Long.toString(now-then)+" ms.");
                }
                if (both) {
    ...
    void doParse(boolean doSparse, XMLReader parser, String xml) throws SAXException, IOException {
        ...
        if (doSparse) {
            if (xml == null)
                xml = slurp(input);
            // Really only for SParse.  This interface intended for a system entity dereference (i.e. a file).
            parser.parse(xml);
        } else {
    ...
    String xmlPreamble;
    class XmlAttribute {
        public XmlAttribute(String ns, String ln, String qn, String val) { this.ns = ns; this.ln=ln; this.qn = qn; value = val; }
        public String ns;
        public String ln;
        public String qn;
        public String value;
    ...
    class Handler implements ContentHandler, LexicalHandler {
        ...
        public void startElement(String ns, String ln, String qn, Attributes atts) throws SAXException {
            ...
            Map.Entry<Xml,Map.Entry<String,String>> childKey = new MapEntry<Xml,Map.Entry<String,String>>(parent,entry);
            if (parent == null || doc == null) doc = newXml; // Root element
            int childKeyHash = childKey.hashCode();
            int newXmlHash = newXml.hashCode();
            if (debug) verbose("startElement: "+ns+" "+ns2q(ns)+":"+ln+", "+qn+" "+childKeyHash);
            ...
            if (existing == null) { // First child with this ns:ln.
                ...
                if (clsn == null)
                    clsn = existing; // If there was only 1 so far, use the one linked to from parent.
		...
		if (parentsLastChild != null) {
		    ...
		    if (debug) verbose("siblings.put:parentsLastChild:"+parentsLastChild.hashCode()+", newXml:"+newXmlHash);
		...
		currentLastChild.put(parent,newXml);
            ...
            if (atts != null && atts.getLength() > 0) {
                ...
                for (int x = 0; x < attrLen; x++) {
                    ...
                    int col;
                    // Even after flipping on both namespace features, Apache SAX still doesn't decode these for xmlns...
                    if (ans.length() == 0 && aln.length() == 0 && aqn.charAt(0)=='x') {
                        if (aqn.startsWith("xmlns:") || aqn.equals("xmlns"))
                            aln = aqn; else {
                            col = aqn.indexOf(':');
                            if (col > -1) {
        ...
        public void endElement(String ns, String ln, String qn) throws SAXException{
            ...
            stack.peek().setEndChar(rawPos); // Character after last character of raw XML bytes for this element.
            stack.pop();
        }
        public void characters(char[] chars, int start, int length) throws SAXException {
            ...
            if (sb == null) {
                ...
                text.put(head, sb);
                if (debug) verbose("characters: text.put:"+head.hashCode()+", "+new String(chars, start, length));
            }
         ...
         */
        int rawPosition() { return rawBuf.length(); } // rawBuf.position(); }
    ...
    private void rawSaveEscaped(String str) {
        ...
        /*
          int len = str.length(), pos=0, x;
          // Do a first pass to see if creating a new object is necessary.
          boolean haveTo = false;
          for (x=0;!haveTo && x < len; x++)
          switch(str.charAt(x)) {
          case '\"':
          case '&':
          case '\'':
          case '<':
          case '>': pos = x; haveTo = true; break;
          }
          if (!haveTo) rawBuf.append(str); else {
          if (pos > 0) rawBuf.append(str.substring(0,pos));
          for (x=pos;x < len; x++) {
          ...
          if (newstr != null) rawBuf.append(newstr); else rawBuf.append(ch);
          }
          }*/
    }
    private void rawSaveEscaped(char[] chars, int start, int length) {
        ...
        if (!haveTo) rawBuf.append(chars, start, length); else {
    ...
    private void rawSaveElement(String ns, String ln, XmlAttribute[] atts) {
        rawBuf.append("<");
        if (ns == null) rawBuf.append(ln); else ns2qnRawSave(ns, ln);
        for (int x=0; atts != null && x < atts.length; x++) {
            rawBuf.append(" ");
            ln = atts[x].ln;
            // Normally we look up ns->prefix, but not for xmlns!
            if (!(ln.equals("xmlns") || ln.startsWith("xmlns:")))
                ns2qnRawSave(atts[x].ns, ln);
            rawBuf.append("=\"");
    ...
    /*
      ...
      if (newstr != null) sb.append(newstr); else sb.append(ch);
      }
      return sb.toString();
      }
    */
	...
	public Xml addChild(String qn, String txt) {
	    ...
	    if ((col = qn.indexOf(':')) > -1) { ns = qn.substring(0,col); ln = qn.substring(col+1); }
            ...
            Map.Entry<String,String> entry = new MapEntry<String,String>(ns, ln);
            Map.Entry<Xml,Map.Entry<String,String>> childKey = new MapEntry<Xml,Map.Entry<String,String>>(this,entry);
	    // ...
            return this;
	}
	...
	// Add Xml as: additional child, only child, to replace current node, or to add/replace children with children of node
	...
	// End modification methods.
        ...
        public int compareTo(Object ol) {
            if (((Xml)ol).serial == serial) return 0;
            if (serial > ((Xml)ol).serial) return 1; else return -1;
        }
	// Every Xml node is unique.  And must be.
        private synchronized long incrementSerial() { return XmlserialMaster++; }
        public int hashCode() { return (int)serial; } // Should ^ upper / lower...
        // Another Xml is only equal to this one if it is really the same node.
        public boolean equals(Object o) {
            if (o instanceof Xml) {
                Xml ox = (Xml)o;
                if (nsi==null ? ox.nsi==null : nsi.equals(ox.nsi) &&
                    lni==null ? ox.lni==null : lni.equals(ox.lni) &&
                    serial == ox.serial) return true; else return false;
            } else return false;
        }
		...
		return xs;
            ...
            if (debug) verbose("characters: text.get:"+this.hashCode()+", "+(sb==null?"null":sb.toString()));
            if (sb == null) return "";
        ...
        public String nss2Xml() throws UnsupportedEncodingException {
            // if (this == doc) return new String(rawBuf.array(), 0, startChar, /*UTF8_CHARSET*/"UTF-8"); // If we're at root, don't need to wrap.
            if (this == doc) if (sparser==null) return rawBuf.substring(0,startChar).toString(); else
                    return new String(sparser.xmlChars,0,startChar);
            if (xmlPreamble == null) { // cache
                StringBuilder nx = new StringBuilder("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><xml");
                // boolean doneOnce = false;
                for (Map.Entry<String,String> pair : ssn.entrySet()) {
                    // if (!doneOnce) { nx.append(" "); doneOnce = true; }
                    nx.append(" xmlns" + (pair.getKey().length()>0?(":"+pair.getKey()):"") + "=\"" + pair.getValue() + "\"");
                }
                nx.append(">");
        ...
        /**
         * Return XML, with generic '<xml>' plus namespace decls for this element, original if not changed.
         * This is parseable later, including the same namespaces, with an extra '/xml/' element and missing any real parents.
         */
        ...
        public String toXmlFragment() throws UnsupportedEncodingException {
            ...
            try {
                if (sparser!=null) res = new String(sparser.xmlChars,startChar,endChar-startChar+1); else
                    res = rawBuf.substring(startChar, endChar).toString();
            } catch (Exception e) {
                e.printStackTrace();
            }
            return res;
        ...
        public Xml getNode(String ns, String localName) {
            ...
            if (localName.indexOf('@') != 0) {
                ...
                // i.e. allow searching for either element or attribute with '/name' syntax, but try element first.
                // This is a programming convenience and one reasonable XML idiom, not anything that breaks the XML format.
                // The idea is that a terminal value could reasonbly be an element with text or an attribute.
                if ((slash = localName.indexOf('@', pos)) > -1)
                    tmpx = getChildNode(curx, ns, localName.substring(pos, slash)); else
                    tmpx = getChildNode(curx, ns, localName.substring(pos));
                if (tmpx != null) {
                    if (slash == -1) { // B) We have a final element, just return
                        return tmpx;
                    } else pos = slash+1; // A) eName@aName -> attribute
                    curx = tmpx;
                } // else C) we have a name with no separators, but it is not an element, assume it might refer to an attribute.
            ...
            XmlAttribute attrs[] = attributes.get(curx);
            if (attrs != null) {
        ...
        public Xml getChildNode(Xml start, String ns, String localName) {
            if (ns == null) {
                int col;
                if ((col = localName.indexOf(':')) < 0) ns = ""; else {
            ...
            int hash = new MapEntry<Xml,
                Map.Entry<String,String>>(start,new MapEntry<String,String>(ns, localName)).hashCode();
        ...
        /**
         * Get the value of a qname.  I.e., single path that might include nsnames.
         * Each level might include a ns!
         */
    ...
    private String ns2q(String ns) {
        String q = nss.get(ns);
        if (q == null) return ns; else return q;
    }
    /**
     * go from namespace id to namespace URI.
     */
    private String q2ns(String q) {
        String ns = ssn.get(q);
        if (ns == null) return q; else return ns;
    }
    /**
    ...
    private String ns2qnHide(String ns, String ln) {
        String q = nss.get(ns);
        if (q == null || q.length() == 0) return ln; else return q+":"+ln;
    }
    static boolean debug = false;
    static boolean verbose = false;
    public static void setDebug(boolean deb, boolean verb) { debug = deb; verbose = verb; }
    public void message(String msg) { if (debug) System.out.println(msg); }
    ...
    public class StackMap<K,V> implements Map<K,V> {
        ...
        public void push(String ename) { smap.addFirst(new /* Tree */ TreeMap<K,V>()); sstack.addFirst(ename); }
        ...
        public String pop() { smap.removeFirst(); String s=sstack.removeFirst(); return s; }
        public String peek() { return sstack.getFirst(); }
        public int levels() { return smap.size(); }
        
        public void clear() { for (Map<K,V> m : smap) m.clear(); }
        public boolean containsKey(Object key) { for (Map<K,V> m : smap) if (m.containsKey(key)) return true; return false; }
        public boolean containsValue(Object val) { for (Map<K,V> m : smap) if (m.containsValue(val)) return true; return false; }
        public Set<Map.Entry<K,V>> entrySet() { return null; } // unused.
        public boolean equals(Object o) { if (o == this) return true; return false; } // unused.
        public V get(Object key) { V t; for (Map<K,V> m : smap) if ((t=m.get(key))!=null) return t; return null; }
        public int hashCode() { int hash=0; for (Map<K,V> m : smap) hash = hash ^ m.hashCode(); return hash; }
        public boolean isEmpty() { for (Map<K,V> m : smap) if (!m.isEmpty()) return false; return true; }
        public Set<K> keySet() { return null; }
        public V put (K key, V val) { return smap.getFirst().put(key, val); }
        public void putAll(Map<? extends K, ? extends V> mn) { for (Map.Entry<? extends K,? extends V> m : mn.entrySet())
                smap.getFirst().put(m.getKey(), m.getValue()); }
        public V remove (Object key) { for (Map<K,V> m : smap) if (!m.containsKey(key)) { V v = m.remove(key); return v; } return null; }
        public int size() { int sz=0; for (Map<K,V> m : smap) sz = sz + m.size(); return sz; }
        public Collection<V> values() { return null; } // unused.
    }
    ...
    public static String slurp(InputStream in) throws IOException {
        ...
        for (int n; (n = in.read(b)) != -1;) {
            // info("Slurp got:"+new String(b, 0, n));
    ...
    /**
     * urlEncoder that works just like java.net.URLEncoder.encode, except that ' '->'%20'. And [.-~_] not [.-*_].
     * And we assume UTF-8.
     */
        ...
        for (int x = encode; x < slen; x++) {
            if (!needsEncode(s.charAt(x))) sb.append(s.charAt(x)); else {
                ...
                for (int y = 0; bs != null && y < bs.length; y++)
                    sb.append("%"+"0123456789ABCDEF".charAt(bs[y]>>4)+"0123456789ABCDEF".charAt(bs[y] & 0x0f));
            }
        }
...
/* Todo notes:
 * Doesn't handle what would be edge cases for most applications: attribute prefixes defined earlier in the same element.
 */
